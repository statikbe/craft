...
export class ImageModalPlugin implements ModalPlugin {
  ...
  public afterCreateModal() {
    ...
    this.modalComponent.modalContent.insertAdjacentElement('beforeend', this.caption);
  }
  ...
  public openPluginModal({ src, caption }) {
    ...
    this.image.classList.add('hidden');
    this.modalComponent.modalContent.insertAdjacentElement('afterbegin', this.image);
    ...
    if (this.options.resizeDuration === 0 || !newImage) {
      ...
      AnimationHelper.cssPropertyAnimation(
        ...
        () => {
          ...
          AnimationHelper.cssPropertyAnimation(this.caption, 'opacity', 1, '', this.options.fadeDuration);
        }
      );
    }
  }
