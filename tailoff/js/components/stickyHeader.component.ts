export class StickyHeader {
  ...
  constructor() {
    ...
    if (this.header) {
      ...
      if (!this.onlyShowOnScrollUp) {
        ...
        this.header.parentElement.style.paddingTop = `${this.headerHeight}px`;
      }
  ...
  private onPageScroll(e) {
    ...
    if (this.onlyShowOnScrollUp) {
      if (top > this.headerHeight && !this.body.classList.contains('header-out-of-view')) {
      ...
      if (
        ((!this.dropInPlace && top <= this.headerHeight) || (this.dropInPlace && top <= 0)) &&
        ...
        this.header.parentElement.style.paddingTop = '';
      }
