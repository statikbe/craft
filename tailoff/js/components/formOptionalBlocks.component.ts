...
export class FormOptionalBlocks {
  constructor() {
    ...
    const optionalRequired = Array.from(
      document.querySelectorAll(".js-form-optional-required")
    );
...
class OptionalBlock {
  ...
  constructor(element: HTMLElement, index) {
    ...
    if (!controllerName) {
      console.error(
        `Make sure you define"data-controller-name" on your optional block`
      );
      return;
    }
...
class OptionalRequired {
  ...
  constructor(element: HTMLElement, index) {
    ...
    try {
      this.controllerValue = JSON.parse(
        element.getAttribute("data-required-value")
      );
    } catch (error) {
