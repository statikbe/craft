    ...
    Array.from(document.querySelectorAll('.js-pull-out')).forEach((block: HTMLElement) => {
      ...
      if (direction) {
        ...
        const paddingLeft = parseInt(
          window.getComputedStyle(block.parentElement, null).getPropertyValue('padding-left')
        );
        const paddingRight = parseInt(
          window.getComputedStyle(block.parentElement, null).getPropertyValue('padding-right')
        );
