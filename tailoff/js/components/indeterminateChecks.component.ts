...
class IndeterminateChecks {
  ...
  private initSubLists() {
    Array.from(this.mainList.querySelectorAll('ul.js-indeterminate-sub-list')).forEach(
  ...
  private checkUp(check: HTMLInputElement) {
    ...
    if (list) {
      ...
      Array.from(this.mainList.querySelectorAll(`#${list.getAttribute('id')} > li > *:not(ul) input`)).forEach(
      ...
      const parentInput = list.closest('li').querySelector('input[type=checkbox]') as HTMLInputElement;
  ...
  private initToggles() {
    const toggles = Array.from(this.mainList.querySelectorAll('.js-indeterminate-toggle'));
    if (toggles.length > 0) {
      ...
      if (this.mainList.getAttribute('data-s-indeterminate-toggle-open') != null) {
        if (this.mainList.getAttribute('data-s-indeterminate-toggle-open') == 'false') {
          Array.from(this.mainList.querySelectorAll('ul')).forEach((list) => {
            const toggle = list.closest('li').querySelector('.js-indeterminate-toggle');
            toggle.setAttribute('aria-expanded', 'true');

            if (this.mainList.getAttribute('data-s-indeterminate-open-checked')) {
