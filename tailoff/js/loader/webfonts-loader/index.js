...
var hashFiles = function (files, hashLength) {
    hashLength =
        hashLength && +hashLength >= 8 && +hashLength <= 32 ? +hashLength : 20;
...
function getFilesAndDeps(patterns, context) {
    ...
    function addByGlob(globExp) {
        ...
        directoryDeps = directoryDeps.concat(
            globDirs.map((file) => {
                return path.resolve(context, file);
            })
        );
...
module.exports = function (content) {
    ...
    var generatorOptions = {
        ...
        templateOptions: {
            ...
            classPrefix:
                "classPrefix" in fontConfig ? fontConfig.classPrefix : "icon-",
        },
        ...
        cssDest: fontConfig.cssDest
            ? path.resolve(
                  this.context,
                  fontConfig.cssDest,
                  fontConfig.fontName.toLowerCase() + ".css"
              )
            : undefined,
    ...
    if (fontConfig.cssTemplate) {
        generatorOptions.cssTemplate = path.resolve(
            this.context,
            fontConfig.cssTemplate
        );
    }

    if (fontConfig.cssFontsUrl) {
        generatorOptions.cssFontsUrl = path.resolve(
            this.context,
            fontConfig.cssFontsUrl
        );
    }

    if (fontConfig.htmlTemplate) {
        generatorOptions.htmlTemplate = path.resolve(
            this.context,
            fontConfig.htmlTemplate
        );
    }

    if (fontConfig.htmlDest) {
        generatorOptions.htmlDest = path.resolve(
            this.context,
            fontConfig.htmlDest
        );
    }
    ...
    if (fontConfig.cssDest && fontConfig.scssFile) {
        generatorOptions.cssDest = path.resolve(
            this.context,
            fontConfig.cssDest,
            fontConfig.fontName.toLowerCase() + ".scss"
        );
    }
    ...
    webfontsGenerator(generatorOptions, (err, res) => {
        ...
        for (var i in formats) {
            ...
            var filename =
                fontConfig.fileName ||
                options.fileName ||
                "[chunkhash]-[fontname].[ext]";
            var chunkHash =
                filename.indexOf("[chunkhash]") !== -1
            ...
            if (!embed) {
                var formatFilename = loaderUtils.interpolateName(
                    this,
                    filename,
                    {
                        context:
                ...
                urls[format] = url.resolve(
                    publicPath,
                    formatFilename.replace(/\\/g, "/")
                );
        ...
        var emitCodepointsOptions =
            fontConfig.emitCodepoints || options.emitCodepoints || null;
        if (emitCodepointsOptions) {
            var emitCodepoints = require("./emit-codepoints");
            emitCodepoints.emitFiles(
                this,
                emitCodepointsOptions,
                generatorOptions,
                options
            );
        }
