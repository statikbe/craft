...
var resolveOptions = (options) => {
  ...
  return {
    ...
    watch: {
      ...
      copyToDest: ((_c = options.watch) == null ? void 0 : _c.copyToDest) ?? false
    }
  };
};
...
var collectCopyTargets = async (root, targets, flatten) => {
  ...
  for (const target of targets) {
    ...
    for (const matchedPath of matchedPaths) {
      ...
      copyTargets.push({
        src: matchedPath,
        dest: import_node_path.default.join(
          destDir,
...
var copyFile = async (file, rootSrc, rootDest, targets, flatten) => {
  const copyTargets = await collectCopyTargets(rootSrc, targets, flatten);
  const changedFiles = [];
  for (const copyTarget of copyTargets) {
    const { src, dest, transform, preserveTimestamps, dereference } = copyTarget;
    const relative = import_node_path.default.relative(file, src);
    if (relative.length == 0 || relative.startsWith("..") && relative.endsWith("..")) {
      const resolvedSrc = import_node_path.default.resolve(rootSrc, file);
      let resolvedDest = import_node_path.default.resolve(rootSrc, rootDest, dest);
      const transformOption = resolveTransformOption(transform);
      if (import_fs_extra.default.statSync(resolvedDest).isDirectory()) {
        resolvedDest = resolvedDest + "/" + import_node_path.default.relative(src, file);
      }
      if (transformOption) {
        await transformCopy(transformOption, resolvedSrc, resolvedDest);
      } else {
      ...
      changedFiles.push(
        import_node_path.default.normalize(rootDest + "/" + dest + "/" + import_node_path.default.relative(src, file))
      );
    }
  }
  return changedFiles;
};
var removeFile = async (file, rootSrc, rootDest, targets, flatten) => {
  const copyTargets = await collectCopyTargets(rootSrc, targets, flatten);
  const changedFiles = [];
  for (const copyTarget of copyTargets) {
    ...
    if (relative.length == 0 || relative.startsWith("..") && relative.endsWith("..")) {
      ...
      await import_fs_extra.default.remove(resolvedDest);
      changedFiles.push(
        import_node_path.default.normalize(rootDest + "/" + dest + "/" + import_node_path.default.relative(src, file))
      );
    }
...
var outputCollectedLog = (logger, collectedMap) => {
  if (collectedMap.size > 0) {
    logger.info(
      formatConsole(import_picocolors.default.green(`Collected ${collectedMap.size} items.`))
    );
  ...
  return {
    ...
    configureServer({ httpServer, middlewares, ws }) {
      ...
      watcher.on("add", async (path2) => {
        config.logger.info(
          formatConsole(`${import_picocolors2.default.green("detected new file")} ${path2}`),
          {
            timestamp: true
          }
        );
        ...
        if (watch.copyToDest) {
          ...
          config.logger.info(
            formatConsole(
              `${import_picocolors2.default.green("file added and copied")} ${path2} ${import_picocolors2.default.green(
                "->"
              )} ${dest.join(import_picocolors2.default.green(", "))}`
            ),
            {
              timestamp: true
            }
      ...
      if (watch.copyToDest) {
        watcher.on("change", async (path2) => {
          ...
          config.logger.info(
            formatConsole(
              `${import_picocolors2.default.green("file changed and copied")} ${path2} ${import_picocolors2.default.green(
                "->"
              )} ${dest.join(import_picocolors2.default.green(", "))}`
            ),
            {
        ...
        watcher.on("unlink", async (path2) => {
          ...
          if (dest.length) {
            config.logger.info(
              formatConsole(
                `${import_picocolors2.default.green("file deleted and removed")} ${path2} ${import_picocolors2.default.green(
                  "and"
                )} ${dest.join(import_picocolors2.default.green(", "))}`
              ),
              {
      ...
      if (watch.reloadPageOnChange && !watch.copyToDest) {
        watcher.on("change", (path2) => {
          config.logger.info(
            formatConsole(`${import_picocolors2.default.green("file changed")} ${path2}`),
            {
              timestamp: true
            }
          );
        ...
        watcher.on("unlink", (path2) => {
          config.logger.info(
            formatConsole(`${import_picocolors2.default.green("file deleted")} ${path2}`),
            {
              timestamp: true
            }
          );
