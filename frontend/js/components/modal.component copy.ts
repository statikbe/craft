...
export default class ModalComponent {
  ...
  private openModalClick(trigger: HTMLElement) {
    ...
    if (this.trigger.getAttribute('data-modal').length > 0) {
      const dialog = document.getElementById(this.trigger.getAttribute('data-modal')) as HTMLDialogElement;
      if (dialog) {
        this.openInlineModal(dialog);
      }
    }
  }
  ...
  public openInlineModal(dialog: HTMLDialogElement) {
    dialog.showModal();
