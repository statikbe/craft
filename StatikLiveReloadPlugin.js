/* jshint node:true */
// Based on https://github.com/statianzo/webpack-livereload-plugin
...
const PLUGIN_NAME = 'StatikLiveReloadPlugin';

class StatikLiveReloadPlugin {
  constructor(options = {}) {
  ...
  _applyCompilation(compilation) {
    ...
    this.webpack.javascript.JavascriptModulesPlugin.getCompilationHooks(compilation).renderRequire.tap(
      PLUGIN_NAME,
      this._scriptTag.bind(this)
    );
  }
  ...
  _afterEmit(compilation) {
    ...
    const include = Object.entries(compilation.assets).map((data) => data[0]);
    if (this._isRunning() && include.length > 0) {
      this.lastHash = hash;
  ...
  _calculateSourceHash(data) {
    ...
    this.sourceHashs[data[0]] = {
      ...
      calculated: StatikLiveReloadPlugin.generateHashCode(data[1].source()),
    };
  }
...
module.exports = StatikLiveReloadPlugin;
